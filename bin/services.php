...
class encryption_services{
    ...
    public function decryption_threading($data,$algorithm="sha256"){
        ...
        $output = ""; $data = str_split($enc_string); 
        $data_contents = []; $i = 4; $x=0;$x_2=0;$x_3=0;$x_4=0; $construct_data = "";$construct_data2 = "";$construct_data3 = "";$construct_data4 = ""; 
        $salt_enc_data = []; $pepering_enc_data = []; $original_enc_data = []; $index_enc_data = []; 
        foreach ($data as $e) { 
            if ($i == 1){
                $x ++; 
                if (($x > $hash_limit) || (strlen($construct_data) >= $hash_limit)){
                    $data_contents[1] = $construct_data; $salt_enc_data[] = $construct_data; $x = 0;  $construct_data = $e; 
                }else{  
                    $construct_data .= $e;  
                }
            }else if ($i == 2){
                ...
                if (($x_2 > $hash_limit) || (strlen($construct_data2) >= $hash_limit)){
                    $data_contents[2] = $construct_data2; $original_enc_data[] = $construct_data2; $x_2 = 0;  $construct_data2 = $e; 
                }else{
                    $construct_data2 .= $e;  
                }
            }else if ($i == 3){
                ...
                if (($x_3 > $hash_limit) || (strlen($construct_data3) >= $hash_limit)){
                    $index_enc_data[] = $construct_data3; $data_contents[3] = $construct_data3; $x_3 = 0; $construct_data3 = $e; 
                }else{
                    $construct_data3 .= $e;  
                }        
            }else if ($i >= 4){
                ...
                if (($x_4 > $hash_limit) || (strlen($construct_data4) >= $hash_limit)){
                    $data_contents[4] = $construct_data4; $pepering_enc_data[] = $construct_data4; $x_4 = 0; $construct_data4 = $e; 
                }else{
                    $construct_data4 .= $e;  
                }
                $i = 0; 
        ...
        $flag = false; $k = -1;  $pattern = $this->get_pattern(); $encryption_guide = str_split($pattern); $encryption_map_guide = []; 
            
        $data_set = []; $index_enc_data = $original_enc_data; 
        while ($flag == false){
            $k ++;
